...
class DungeonLevelSystem:
    ...
    def generate_level_with_monsters(
        self, level_number: int
    ) -> Tuple[GameMap, List[Room], List[Entity], Position]:
        ...
        Returns:
            Tuple of (game map, list of rooms, list of entities (monsters + items), stairs position)
        """
        config = self.level_configs[level_number]
        ...
        for room in rooms:
            room_items = place_items(room, max_items=max_items_per_room)
